...
The /CNOT/ gate is a Controlled-NOT operation or, what is the same, a Pauli-x gate that, depending on the sate of the control qubit will be executed or not.
As the CNOT, the /CZ/ gate is a Controlled-Z operations that, in this case, executes a Pauli-z gates depending on the state of the control qubit.
...
\end{figure}
#+END_EXPORT
*** TODO Quantum Error Correction (QEC) and Fault Tolerant (FT) quantum computation
