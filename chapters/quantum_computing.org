...
# Multiple qubits [Entanglement, Math operation (convolution), Hilbert space increments] # Density matrices
...
In eq. [[eq:outer_product]] and [[eq:convolution_example_entanglement]] an example of the outer product is offered.
...
Another use of the outer product is the calculation of the *density matrices*.
A density matrix is a state representation that is calculated as the outer product of the qubit state with itself $| \psi \rangle \,\langle \psi |$ as in eq. [[eq:outer_product]].
