...
In order to address the mapping problem, we split it in three steps, namely: *scheduling*, *initial placement* and *routing*.
...
Throughout this thesis we use the terms */virtual/ qubits* and */physical/ qubits* referring to the qubits from the desired circuit to map and the qubits in the quantum chip, respectively.
