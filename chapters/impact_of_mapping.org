...
We calculate the percentage of the fidelity difference as $\left(\frac{f_{\text{before}} - f_{\text{after}}}{f_{\text{before}}}\right)$ where $f_{\text{after}}$ is the fidelity of the mapped circuit and f_{\text{before}} is the fidelity of the non-mapped one.
