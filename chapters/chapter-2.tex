...
\label{sec:org8e30225}

\section*{Quantum computing}
\label{sec:org9ce632c}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:org91d1192}.
...
E.g. in eq. \ref{eq:orgb62c808} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:org08ec97c}).
...
In eq. \ref{eq:org09abba5} and \ref{eq:org3dab82f} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:orgbf533a0} \ldots{}
...
\label{sec:org4c541e9}

\item Two-qubit gates
\label{sec:orgb016e4e}
\end{itemize}
\end{itemize}

\subsection*{Quantum Circuits}
\label{sec:orgb285bae}
...
An example of the Gray code is shown in \ref{fig:org45d4810} to recall how the Gray code is.
The QASM algorithm representation can be seen in code \ref{orgfbf486c}.
...
\end{minted}
\caption{\label{orgfbf486c}
QASM code describing the Gray code algorithm.}
\end{listing}
...
\label{sec:org4fa44e7}

\subsection*{Mapping}
\label{sec:org9a2415f}
...
\label{sec:orga0b675a}

\begin{itemize}
\item Example
\label{sec:orgc9b3315}
\begin{itemize}
\item Original
\label{sec:org3a234a8}
...
\label{sec:org51de34a}
\begin{itemize}
\item :BMCOL:
\label{sec:orgd983d5c}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org67c323d}
...
\label{sec:org3b818c6}


\begin{itemize}
\item Example definition
\label{sec:org1e87d98}
\begin{itemize}
\item Example definition
\label{sec:orgb76cdf6}
\begin{center}
...
\label{sec:org22ab59a}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org1b17341}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org6ae8d65}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org4391fb4}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgf6c643e}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org961929f}
...
\label{sec:orge48de91}
\begin{itemize}
\item Example definition
\label{sec:orge40b349}
\begin{itemize}
\item Example definition
\label{sec:orgba3e45f}
\begin{center}
...
\label{sec:org24392fd}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org859367c}
\begin{center}
...
\item :BMCOL:
\label{sec:org6af336d}



\item Gray Code Quantum Circuit
\label{sec:org0fb658a}
\uline{Gray Code Quantum Circuit}
...
\label{sec:org420b575}
\begin{itemize}
\item Example definition
\label{sec:orgcc708f2}
\begin{itemize}
\item Example definition
\label{sec:org88dfa8e}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org6761c83}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org2c2dd69}
\begin{center}
...
\item :BMCOL:
\label{sec:org96e87e3}



\item Gray Code Quantum Circuit
\label{sec:orgce22f34}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org6d51304}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org242639f}
\begin{itemize}
\item Example definition
\label{sec:org442d529}
\begin{itemize}
\item Example definition
\label{sec:org2c8fb35}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org44d111e}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgea54f0f}
\begin{center}
...
\item :BMCOL:
\label{sec:org9189448}



\item Gray Code Quantum Circuit
\label{sec:org3a6a77d}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgeda14c1}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgb31d2cc}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgcead9d8}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:org4f3512a}

\begin{itemize}
\item Circuits
\label{sec:orgc4984d0}
\begin{itemize}
\item :BMCOL:
\label{sec:org07f274e}
      \begin{center}
...
\label{sec:orgbdac4b6}

\begin{itemize}
\item :BMCOL:
\label{sec:orgd411a9d}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org063f036}
Error sources:
...
\item :BMCOL:
\label{sec:orgb29050e}
\begin{itemize}
\item Windows error image
\label{sec:org1201846}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org8344e25}

\item Best Mapping
\label{sec:org29e5b45}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgb4d6b65}
...
\label{sec:orgdb5f8ca}

\begin{itemize}
\item Index
\label{sec:orgb02c56f}
\begin{itemize}
\item What is the people doing?
\label{sec:org278e93a}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org94213f7}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgf1e4df5}
...
\label{sec:orgbf1d36f}

\begin{itemize}
\item SC-17 topology
\label{sec:org0b2b7f0}
\begin{itemize}
\item Images
\label{sec:orgcc161c8}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:org6c9a2a5}
...
\item Constraints
\label{sec:org0d3d90a}
\begin{itemize}
\item :BMCOL:
\label{sec:org76d7e87}
Constraints:
