...
\label{sec:orgcb8811b}

\section*{Quantum computing}
\label{sec:orge26a9ff}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:orgfbbb9f2}.
...
E.g. in eq. \ref{eq:org2e7596e} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:orgb8e499a}).
...
In eq. \ref{eq:org846f2f4} and \ref{eq:org35dc5f4} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:org65b1658} one can understand how some uniform operation \(U\) is applied to a qubit with state \(| \psi \rangle\).
...
Both representations of the most common single-qubit gates can be found in Table \ref{tab:orgc9c79c2}.

\begin{table}[htbp]
\caption{\label{tab:orgc9c79c2}
...
\label{sec:org6710a0e}

\subsection*{Mapping}
\label{sec:org2dca9a5}
...
\label{sec:orgbc8d203}

\begin{itemize}
\item Example
\label{sec:org96c414f}
\begin{itemize}
\item Original
\label{sec:orga39758b}
...
\label{sec:org20bc7e4}
\begin{itemize}
\item :BMCOL:
\label{sec:orgbd82457}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org6943934}
...
\label{sec:org7786876}


\begin{itemize}
\item Example definition
\label{sec:orgf789f06}
\begin{itemize}
\item Example definition
\label{sec:orgd9cbfab}
\begin{center}
...
\label{sec:org36c7e77}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org812649f}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:orge6f70e5}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgb2b44f0}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org2810027}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org0636e80}
...
\label{sec:org868ffae}
\begin{itemize}
\item Example definition
\label{sec:orga9e93c7}
\begin{itemize}
\item Example definition
\label{sec:org9c88fc8}
\begin{center}
...
\label{sec:org2384d1a}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org916e9b6}
\begin{center}
...
\item :BMCOL:
\label{sec:org9792611}



\item Gray Code Quantum Circuit
\label{sec:org94b2f31}
\uline{Gray Code Quantum Circuit}
...
\label{sec:orgeabc2da}
\begin{itemize}
\item Example definition
\label{sec:org7d86e8f}
\begin{itemize}
\item Example definition
\label{sec:orgeb51873}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgf22ea3e}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orga4439b1}
\begin{center}
...
\item :BMCOL:
\label{sec:orgbf20e6c}



\item Gray Code Quantum Circuit
\label{sec:org3cb7bac}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org17dd4a3}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org704e25b}
\begin{itemize}
\item Example definition
\label{sec:org8b5e084}
\begin{itemize}
\item Example definition
\label{sec:orgdc6518b}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org6fb3f33}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgd1d7628}
\begin{center}
...
\item :BMCOL:
\label{sec:org58a021b}



\item Gray Code Quantum Circuit
\label{sec:org347ebc2}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org5803279}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgca47641}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:org9f12549}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:org175e106}

\begin{itemize}
\item Circuits
\label{sec:orgd7bc29b}
\begin{itemize}
\item :BMCOL:
\label{sec:orgf3227cc}
      \begin{center}
...
\label{sec:org1e8d768}

\begin{itemize}
\item :BMCOL:
\label{sec:orge65e2f9}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org31f058a}
Error sources:
...
\item :BMCOL:
\label{sec:org6b15c94}
\begin{itemize}
\item Windows error image
\label{sec:org05c6148}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org595820f}

\item Best Mapping
\label{sec:org215222b}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org570c488}
...
\label{sec:orgf6a212e}

\begin{itemize}
\item Index
\label{sec:org08109ce}
\begin{itemize}
\item What is the people doing?
\label{sec:org3242f56}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orge520158}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgc67a469}
...
\label{sec:orgc4794b4}

\begin{itemize}
\item SC-17 topology
\label{sec:orgf7eacba}
\begin{itemize}
\item Images
\label{sec:orge6be3e8}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:orgb711bb7}
...
\item Constraints
\label{sec:org93508fe}
\begin{itemize}
\item :BMCOL:
\label{sec:orgdbdf931}
Constraints:
