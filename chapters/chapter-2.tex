...
\label{sec:org85c1008}

\section*{Quantum computing}
\label{sec:org924203d}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:org075754f}.
...
E.g. in eq. \ref{eq:org9a03a46} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:org4ed6857}).
...
In eq. \ref{eq:orgc4d39d0} and \ref{eq:orgf71a549} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:orgca6a04a} one can understand how some uniform operation \(U\) is applied to a qubit with state \(| \psi \rangle\).
...
An example of the Gray code is shown in \ref{fig:org5739fc7} for different number of bits (\(n\)).
Also, the QASM algorithm representation can be seen in code \ref{org4ed3cdf}.
...
\label{sec:orga19f2a4}

\subsection*{Mapping}
\label{sec:orgc8e25b1}
...
\label{sec:org7937109}

\begin{itemize}
\item Example
\label{sec:org5f8203d}
\begin{itemize}
\item Original
\label{sec:org1ff5a7e}
...
\label{sec:org8a7589b}
\begin{itemize}
\item :BMCOL:
\label{sec:org05e7966}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org8963d28}
...
\label{sec:org44df33b}


\begin{itemize}
\item Example definition
\label{sec:orgb96cb81}
\begin{itemize}
\item Example definition
\label{sec:org96b8c09}
\begin{center}
...
\label{sec:orgec7b995}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orge046f09}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org6b8208a}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org72c1534}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgc90dad8}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orge6df1f8}
...
\label{sec:orgae26b9e}
\begin{itemize}
\item Example definition
\label{sec:orgd407baa}
\begin{itemize}
\item Example definition
\label{sec:orgb41c2d5}
\begin{center}
...
\label{sec:org454dede}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org8e107e3}
\begin{center}
...
\item :BMCOL:
\label{sec:org41a472d}



\item Gray Code Quantum Circuit
\label{sec:org76dccae}
\uline{Gray Code Quantum Circuit}
...
\label{sec:orgddcf7c6}
\begin{itemize}
\item Example definition
\label{sec:orgd810a2c}
\begin{itemize}
\item Example definition
\label{sec:org83f9c0c}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org3a03867}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org90476eb}
\begin{center}
...
\item :BMCOL:
\label{sec:org027bc7f}



\item Gray Code Quantum Circuit
\label{sec:orgf7dbc6a}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org7291b4a}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org1444f9a}
\begin{itemize}
\item Example definition
\label{sec:org0c7d912}
\begin{itemize}
\item Example definition
\label{sec:org0ec2162}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orga8186c9}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgf58ae62}
\begin{center}
...
\item :BMCOL:
\label{sec:org70666d1}



\item Gray Code Quantum Circuit
\label{sec:orgc67da12}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org5127088}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgbeb89e5}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgec9fb7a}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:org46537db}

\begin{itemize}
\item Circuits
\label{sec:org2089191}
\begin{itemize}
\item :BMCOL:
\label{sec:orgaf578f9}
      \begin{center}
...
\label{sec:orgae7f235}

\begin{itemize}
\item :BMCOL:
\label{sec:org094831e}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgcf6bfb2}
Error sources:
...
\item :BMCOL:
\label{sec:orgd187053}
\begin{itemize}
\item Windows error image
\label{sec:orgf9a0c40}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgb9c253f}

\item Best Mapping
\label{sec:org14ac7f3}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org767d854}
...
\label{sec:orgb1e2a68}

\begin{itemize}
\item Index
\label{sec:org7ffdef2}
\begin{itemize}
\item What is the people doing?
\label{sec:org82e91d6}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgf7457ed}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org19b3514}
...
\label{sec:org4063076}

\begin{itemize}
\item SC-17 topology
\label{sec:org620bf05}
\begin{itemize}
\item Images
\label{sec:org27109d5}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:orgab8fc18}
...
\item Constraints
\label{sec:org63dfef8}
\begin{itemize}
\item :BMCOL:
\label{sec:orge26a942}
Constraints:
