...
\label{sec:org1505e64}

\section*{Quantum computing}
\label{sec:org33b3629}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:orga9ff4ed}.
...
E.g. in eq. \ref{eq:org70fca7d} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:org92041e1}).
...
In eq. \ref{eq:org847c27e} and \ref{eq:orge30b011} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:orgaf0e034} one can understand how some uniform operation \(U\) is applied to a qubit with state \(| \psi \rangle\).
...
Both representations of the most common single-qubit gates can be found in the table \ref{tab:org328db75}.

\begin{table}[htbp]
\caption{\label{tab:org328db75}
...
An example of the Gray code is shown in \ref{fig:org3a94b76} for different number of bits (\(n\)).
Also, the QASM algorithm representation can be seen in code \ref{org9b108ec}.
...
\label{sec:orgeecdd6d}

\subsection*{Mapping}
\label{sec:org5162604}
...
\label{sec:org2a76eba}

\begin{itemize}
\item Example
\label{sec:org6276b83}
\begin{itemize}
\item Original
\label{sec:org89c18ba}
...
\label{sec:org4af1205}
\begin{itemize}
\item :BMCOL:
\label{sec:orgba4e4aa}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org6016bee}
...
\label{sec:orge3a0998}


\begin{itemize}
\item Example definition
\label{sec:org1648bf6}
\begin{itemize}
\item Example definition
\label{sec:orgc94b672}
\begin{center}
...
\label{sec:org283828f}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org058b9ca}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org5042ea4}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org3d86cd8}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org7336513}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org324d74c}
...
\label{sec:org99a235c}
\begin{itemize}
\item Example definition
\label{sec:org5f09f39}
\begin{itemize}
\item Example definition
\label{sec:org7e1eb4a}
\begin{center}
...
\label{sec:orgbba5b17}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgdb2fa16}
\begin{center}
...
\item :BMCOL:
\label{sec:orgc96bb10}



\item Gray Code Quantum Circuit
\label{sec:orgb16b846}
\uline{Gray Code Quantum Circuit}
...
\label{sec:org41cc58b}
\begin{itemize}
\item Example definition
\label{sec:orge744f38}
\begin{itemize}
\item Example definition
\label{sec:org4ac6eef}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org797681e}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org177e962}
\begin{center}
...
\item :BMCOL:
\label{sec:org605ff02}



\item Gray Code Quantum Circuit
\label{sec:org7426c01}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org5c4f884}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org0777a09}
\begin{itemize}
\item Example definition
\label{sec:orgb2b0d1e}
\begin{itemize}
\item Example definition
\label{sec:org36e74df}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgca21d54}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org7829043}
\begin{center}
...
\item :BMCOL:
\label{sec:org586657b}



\item Gray Code Quantum Circuit
\label{sec:org3c71929}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org0a4d42e}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgc84e913}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:org64c7376}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:orgc696a65}

\begin{itemize}
\item Circuits
\label{sec:org733ff28}
\begin{itemize}
\item :BMCOL:
\label{sec:org565c404}
      \begin{center}
...
\label{sec:org3aad928}

\begin{itemize}
\item :BMCOL:
\label{sec:orgaf4e85e}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgbb9b5c8}
Error sources:
...
\item :BMCOL:
\label{sec:orge3b4bf6}
\begin{itemize}
\item Windows error image
\label{sec:org508dc52}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org16bb8c6}

\item Best Mapping
\label{sec:org3d49c1f}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org34cd064}
...
\label{sec:orgcc55955}

\begin{itemize}
\item Index
\label{sec:org3906f5a}
\begin{itemize}
\item What is the people doing?
\label{sec:org9f19b4e}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgdd7eb22}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org44592f2}
...
\label{sec:org4da1045}

\begin{itemize}
\item SC-17 topology
\label{sec:orgc05f3f0}
\begin{itemize}
\item Images
\label{sec:org0d7a8b1}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:org1ae9c65}
...
\item Constraints
\label{sec:org7f66d3c}
\begin{itemize}
\item :BMCOL:
\label{sec:orgebbde80}
Constraints:
