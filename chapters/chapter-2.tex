...
\label{sec:org610100b}

\section*{Quantum computing}
\label{sec:orgb2103eb}
...
\begin{itemize}
\item Qubits
\label{sec:org05e137d}

Qubit stands for quantum bit.
A basic unit of information founded on quantum physics laws.
Unlike a classical bit, whose state is either \texttt{0} or \texttt{1}, but like a particle; qubits are not in a fixed state -- ground or excited -- until is measured.
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:orgdbe4da1}.
...
E.g. in eq. \ref{eq:org04467c1} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to get either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:orgd6bdb4d}).
...
In eq. \ref{eq:org08aaa79} an example of the outer product is offered.
...
\item Quantum Gates
\label{sec:org3bec4e0}

\begin{itemize}
\item Single-qubit gates
\label{sec:org7a61388}

\item Two-qubit gates
\label{sec:org9cfe0c4}
\end{itemize}
\end{itemize}
\subsection*{Quantum Circuits}
\label{sec:orgf3bd2b9}

\section*{Mapping of quantum circuits}
\label{sec:org6026031}

\subsection*{Mapping}
\label{sec:orgb5c3264}
...
\label{sec:orge17be82}

\begin{itemize}
\item Example
\label{sec:orga93bc00}
\begin{itemize}
\item Original
\label{sec:org9cb6d0f}
...
\label{sec:orgb311105}
\begin{itemize}
\item :BMCOL:
\label{sec:org50f7b09}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org0ea3c30}
...
\label{sec:org45f3998}


\begin{itemize}
\item Example definition
\label{sec:orgbc92c72}
\begin{itemize}
\item Example definition
\label{sec:org9f4db20}
\begin{center}
...
\label{sec:orgf506ce0}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org59876e0}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org0245ef8}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgacc5ccf}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgff9a0d6}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgab5d4a3}
...
\label{sec:org7e426aa}
\begin{itemize}
\item Example definition
\label{sec:org9c54ea1}
\begin{itemize}
\item Example definition
\label{sec:org4b1b346}
\begin{center}
...
\label{sec:org136a5f0}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org0042472}
\begin{center}
...
\item :BMCOL:
\label{sec:orgeb28e4c}



\item Gray Code Quantum Circuit
\label{sec:org8e5f79e}
\uline{Gray Code Quantum Circuit}
...
\label{sec:orgc2b7dbb}
\begin{itemize}
\item Example definition
\label{sec:org71f0517}
\begin{itemize}
\item Example definition
\label{sec:org86b5c22}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org692dad5}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org5cfeeea}
\begin{center}
...
\item :BMCOL:
\label{sec:org76da1c1}



\item Gray Code Quantum Circuit
\label{sec:orgb7195ce}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org86f26f9}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org710e9e7}
\begin{itemize}
\item Example definition
\label{sec:orgc8bff31}
\begin{itemize}
\item Example definition
\label{sec:org9f1bb0a}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgecf5ada}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org109ed5d}
\begin{center}
...
\item :BMCOL:
\label{sec:orgd4929fd}



\item Gray Code Quantum Circuit
\label{sec:org01b9baa}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgd53ec81}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgd178cac}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:org06dd965}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:org51d5d6f}

\begin{itemize}
\item Circuits
\label{sec:orge2173d6}
\begin{itemize}
\item :BMCOL:
\label{sec:orgad17385}
      \begin{center}
...
\label{sec:orga163761}

\begin{itemize}
\item :BMCOL:
\label{sec:orge07dc63}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org8a2fe1a}
Error sources:
...
\item :BMCOL:
\label{sec:orgd10474e}
\begin{itemize}
\item Windows error image
\label{sec:orgd8e4cb8}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgc25247d}

\item Best Mapping
\label{sec:org28a9933}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orge1edb01}
...
\label{sec:org73cf23c}

\begin{itemize}
\item Index
\label{sec:org694b1d4}
\begin{itemize}
\item What is the people doing?
\label{sec:org9bddda8}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgcb2a296}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgc48ed76}
...
\label{sec:orge38fd1d}

\begin{itemize}
\item SC-17 topology
\label{sec:org5e68a06}
\begin{itemize}
\item Images
\label{sec:org6919de7}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:org93bc9a0}
...
\item Constraints
\label{sec:org1b18736}
\begin{itemize}
\item :BMCOL:
\label{sec:orgace6ee8}
Constraints:
