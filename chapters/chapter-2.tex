...
\label{sec:org018b3e5}

\section*{Quantum computing}
\label{sec:org6b3898b}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:org9a773ca}.
...
E.g. in eq. \ref{eq:org3816e51} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:org858dde7}).
...
In eq. \ref{eq:orgeda78a9} and \ref{eq:org827deb7} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:org0363fd0} \ldots{}
...
\label{sec:org90140c8}

\item Two-qubit gates
\label{sec:org2d99fd3}
\end{itemize}
\end{itemize}

\subsection*{Quantum Circuits}
\label{sec:orga546374}
...
An example of the Gray code is shown in \ref{fig:orgeb81265} to recall how the Gray code is.
The QASM algorithm representation can be seen in code \ref{org1bedbd3}.
...
\label{sec:org589b9fa}

\subsection*{Mapping}
\label{sec:orga712fc6}
...
\label{sec:org35baf6f}

\begin{itemize}
\item Example
\label{sec:org33c4d7f}
\begin{itemize}
\item Original
\label{sec:orgda01800}
...
\label{sec:org1a5a162}
\begin{itemize}
\item :BMCOL:
\label{sec:org1b21a67}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org8102db1}
...
\label{sec:org92df4fc}


\begin{itemize}
\item Example definition
\label{sec:org39915cc}
\begin{itemize}
\item Example definition
\label{sec:org8040369}
\begin{center}
...
\label{sec:org2a5a48c}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org9bf0a07}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org6077712}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org45fba79}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgcfc5097}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org400a6f8}
...
\label{sec:org90b4cb5}
\begin{itemize}
\item Example definition
\label{sec:orgde64c4d}
\begin{itemize}
\item Example definition
\label{sec:org1fb7896}
\begin{center}
...
\label{sec:orgd54e734}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org4712ab6}
\begin{center}
...
\item :BMCOL:
\label{sec:org2f8ab18}



\item Gray Code Quantum Circuit
\label{sec:org97f4356}
\uline{Gray Code Quantum Circuit}
...
\label{sec:orge7cfe0a}
\begin{itemize}
\item Example definition
\label{sec:org3b0b2f5}
\begin{itemize}
\item Example definition
\label{sec:org78a3461}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org095c653}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org9c5596a}
\begin{center}
...
\item :BMCOL:
\label{sec:org31f66b9}



\item Gray Code Quantum Circuit
\label{sec:orgc1c94c9}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org0607aea}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:orgc4d3526}
\begin{itemize}
\item Example definition
\label{sec:orgc5a27d4}
\begin{itemize}
\item Example definition
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgd26bb33}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org1182a6c}
\begin{center}
...
\item :BMCOL:
\label{sec:org6da8c30}



\item Gray Code Quantum Circuit
\label{sec:orga3bb8c6}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org77127c9}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org41a5de4}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:org96a1eba}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:org60b2904}

\begin{itemize}
\item Circuits
\label{sec:org960ef87}
\begin{itemize}
\item :BMCOL:
\label{sec:org846c152}
      \begin{center}
...
\label{sec:org7675947}

\begin{itemize}
\item :BMCOL:
\label{sec:org5122a73}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org13f319b}
Error sources:
...
\item :BMCOL:
\label{sec:orgd992a6d}
\begin{itemize}
\item Windows error image
\label{sec:org266787d}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org0625d78}

\item Best Mapping
\label{sec:org554f4bd}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org4e522b0}
...
\label{sec:orge194527}

\begin{itemize}
\item Index
\label{sec:org561d46b}
\begin{itemize}
\item What is the people doing?
\label{sec:org158f440}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org6f50f8f}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgf42a08b}
...
\label{sec:orgde3ef21}

\begin{itemize}
\item SC-17 topology
\label{sec:org55733b8}
\begin{itemize}
\item Images
\label{sec:orgc75fee2}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:org75d778c}
...
\item Constraints
\label{sec:org8f38283}
\begin{itemize}
\item :BMCOL:
\label{sec:orgd233255}
Constraints:
