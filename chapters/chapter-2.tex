...
\label{sec:org65e6472}

\section*{Quantum computing}
\label{sec:org412400e}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:orgf1f3360}.
...
E.g. in eq. \ref{eq:orga0aadec} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:orge39f916}).
...
In eq. \ref{eq:org512605a} and \ref{eq:org45f4932} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:org188d455} one can understand how some uniform operation \(U\) is applied to a qubit with state \(| \psi \rangle\).
...
Both representations of the most common single-qubit gates can be found in the table \ref{tab:org546debd}.

\begin{table}[htbp]
\caption{\label{tab:org546debd}
...
\begin{bmatrix}1\&0$\backslash$\0\&1\end{bmatrix} & \begin{bmatrix}0\&1$\backslash$\1\&0\end{bmatrix} & \begin{bmatrix}0\&-i$\backslash$\i\&0\end{bmatrix} & \begin{bmatrix}1\&0$\backslash$\0\&-1\end{bmatrix} & \frac{1}\{\sqrt{2}\}\begin{bmatrix}1\&1$\backslash$\1\&-1\end{bmatrix} & \begin{bmatrix}1\&0$\backslash$\0\&i\end{bmatrix} & \begin{bmatrix}1\&0$\backslash$\0\&e\(^{\text{i }\pi\ \text{/ 4}}\)\}\end{bmatrix}\\
\end{tabular}
\end{table}

\item Two-qubit gates
\label{sec:org60c3924}
...
An example of the Gray code is shown in \ref{fig:org3d3f479} for different number of bits (\(n\)).
Also, the QASM algorithm representation can be seen in code \ref{orgcefc5f8}.
...
\label{sec:org2de4eca}

\subsection*{Mapping}
\label{sec:orgbfdc57a}
...
\label{sec:orgdeb1a19}

\begin{itemize}
\item Example
\label{sec:org723019f}
\begin{itemize}
\item Original
\label{sec:orgd7d4a16}
...
\label{sec:orgf6a4713}
\begin{itemize}
\item :BMCOL:
\label{sec:org754beaf}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:orgcbe435f}
...
\label{sec:org415a20d}


\begin{itemize}
\item Example definition
\label{sec:org4887684}
\begin{itemize}
\item Example definition
\label{sec:org5d35ca9}
\begin{center}
...
\label{sec:orge168ac7}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org8f05bc2}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org332ebf0}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orga1211d3}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org2856159}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org1daed93}
...
\label{sec:orgf665f95}
\begin{itemize}
\item Example definition
\label{sec:orgdda09e9}
\begin{itemize}
\item Example definition
\label{sec:org8220ee9}
\begin{center}
...
\label{sec:org187705c}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org060d5aa}
\begin{center}
...
\item :BMCOL:
\label{sec:org1faae15}



\item Gray Code Quantum Circuit
\label{sec:org7ca57bb}
\uline{Gray Code Quantum Circuit}
...
\label{sec:org8d7f7e5}
\begin{itemize}
\item Example definition
\label{sec:orgcf8c6e5}
\begin{itemize}
\item Example definition
\label{sec:org5eea09e}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orge7f8d2b}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgfdf0be0}
\begin{center}
...
\item :BMCOL:
\label{sec:orgbd18fd8}



\item Gray Code Quantum Circuit
\label{sec:orge3ee18b}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgf9f3834}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org68ac9b9}
\begin{itemize}
\item Example definition
\label{sec:org6f0e05e}
\begin{itemize}
\item Example definition
\label{sec:orge132883}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org401fd46}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgd9eb53f}
\begin{center}
...
\item :BMCOL:
\label{sec:orga8af191}



\item Gray Code Quantum Circuit
\label{sec:org704672d}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org272ee8f}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org3538830}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:org90b7a71}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:org9dd4046}

\begin{itemize}
\item Circuits
\label{sec:org7d52aec}
\begin{itemize}
\item :BMCOL:
\label{sec:orgc3cf9eb}
      \begin{center}
...
\label{sec:org6a34c31}

\begin{itemize}
\item :BMCOL:
\label{sec:orgd33a3fb}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgcf57ed7}
Error sources:
...
\item :BMCOL:
\label{sec:org1c47a27}
\begin{itemize}
\item Windows error image
\label{sec:org09824b7}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org74d4b0f}

\item Best Mapping
\label{sec:org6a15c3b}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgfe8ad6d}
...
\label{sec:orgb00d874}

\begin{itemize}
\item Index
\label{sec:orgaf954c3}
\begin{itemize}
\item What is the people doing?
\label{sec:org45cb297}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org8980667}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org97dac17}
...
\label{sec:orgdb35c03}

\begin{itemize}
\item SC-17 topology
\label{sec:orgfccf921}
\begin{itemize}
\item Images
\label{sec:org15c943b}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:orgcf1ab8f}
...
\item Constraints
\label{sec:org0292d3c}
\begin{itemize}
\item :BMCOL:
\label{sec:org7654c2a}
Constraints:
