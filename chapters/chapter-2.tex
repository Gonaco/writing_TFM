...
\label{sec:org9886da0}

\section*{Quantum computing}
\label{sec:org183f385}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:orga72a67a}.
...
E.g. in eq. \ref{eq:orga1be1c7} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:org7612262}).
...
In eq. \ref{eq:org06444a9} and \ref{eq:org0caaa0d} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:org9ca279d} one can understand how some uniform operation \(U\) is applied to a qubit with state \(| \psi \rangle\).
...
An example of the Gray code is shown in \ref{fig:orgf645a78} for different number of bits (\(n\)).
Also, the QASM algorithm representation can be seen in code \ref{orgbac9a7f}.
...
\label{sec:orgb3b32e9}

\subsection*{Mapping}
\label{sec:org389ba77}
...
\label{sec:orgc081d1f}

\begin{itemize}
\item Example
\label{sec:org9bfccbc}
\begin{itemize}
\item Original
\label{sec:org13f5361}
...
\label{sec:org83db66e}
\begin{itemize}
\item :BMCOL:
\label{sec:org4708ef9}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org19d384d}
...
\label{sec:orgd5b2b4b}


\begin{itemize}
\item Example definition
\label{sec:org2419b9b}
\begin{itemize}
\item Example definition
\label{sec:org338c471}
\begin{center}
...
\label{sec:org950b586}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org248582d}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:org91010c0}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org5e4596f}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orgfdac901}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgae0a7fb}
...
\label{sec:org9648d4c}
\begin{itemize}
\item Example definition
\label{sec:orgc805456}
\begin{itemize}
\item Example definition
\label{sec:org1d6876b}
\begin{center}
...
\label{sec:org392c5fa}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org0e8a883}
\begin{center}
...
\item :BMCOL:
\label{sec:orgae6287e}



\item Gray Code Quantum Circuit
\label{sec:orgeb9847f}
\uline{Gray Code Quantum Circuit}
...
\label{sec:org376d588}
\begin{itemize}
\item Example definition
\label{sec:org9526017}
\begin{itemize}
\item Example definition
\label{sec:org8895e80}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org4ce6468}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org17c023e}
\begin{center}
...
\item :BMCOL:
\label{sec:org59aef0b}



\item Gray Code Quantum Circuit
\label{sec:orga09b828}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgebe5e4a}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org93efff8}
\begin{itemize}
\item Example definition
\label{sec:org652e80a}
\begin{itemize}
\item Example definition
\label{sec:org25c505f}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org942d675}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:orgc074641}
\begin{center}
...
\item :BMCOL:
\label{sec:org49323f8}



\item Gray Code Quantum Circuit
\label{sec:org4c44304}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org50a57b4}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org318c20b}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:org7dc16ae}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:orga218b91}

\begin{itemize}
\item Circuits
\label{sec:orgc6e4033}
\begin{itemize}
\item :BMCOL:
\label{sec:orgbf8631e}
      \begin{center}
...
\label{sec:orgf6a4fbb}

\begin{itemize}
\item :BMCOL:
\label{sec:org69ff4de}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org3976c75}
Error sources:
...
\item :BMCOL:
\label{sec:orgb3055d7}
\begin{itemize}
\item Windows error image
\label{sec:org9871e4b}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org148316d}

\item Best Mapping
\label{sec:org13ba14d}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org0d111d4}
...
\label{sec:orgf00ba91}

\begin{itemize}
\item Index
\label{sec:orgbfff6d1}
\begin{itemize}
\item What is the people doing?
\label{sec:org307730e}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org8ad870f}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgfff4405}
...
\label{sec:orgfcaf985}

\begin{itemize}
\item SC-17 topology
\label{sec:org51af47e}
\begin{itemize}
\item Images
\label{sec:org6f9af78}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:orgcc371ce}
...
\item Constraints
\label{sec:org6c351c2}
\begin{itemize}
\item :BMCOL:
\label{sec:orgdda6db4}
Constraints:
