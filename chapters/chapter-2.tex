...
\label{sec:orgd90295d}

\section*{Quantum computing}
\label{sec:orgdc82db4}
...
Dirac's notation grants the definition of the quantum states as vectors, as in eq. \ref{eq:org6b6cac7}.
...
E.g. in eq. \ref{eq:org9bfcd47} the vectors of the ground, excited and the plus superposition -- a quantum state with the same probability to measure either \(|0\rangle\) or \(|1\rangle\) -- states are depicted, respectively.
...
As soon as the vectors are 2-dimensional, complex and unitary they can also be described in phase notation (eq. \ref{eq:org824099c}).
...
In eq. \ref{eq:orgc6fdab3} and \ref{eq:org8e32c51} an example of the outer product is offered.
...
For instance, in eq. \ref{eq:org5a5ee3e} one can understand how some uniform operation \(U\) is applied to a qubit with state \(| \psi \rangle\).
...
An example of the Gray code is shown in \ref{fig:org44d8ab8} for different number of bits (\(n\)).
Also, the QASM algorithm representation can be seen in code \ref{org1b32ac6}.
...
\label{sec:org6d7c165}

\subsection*{Mapping}
\label{sec:org0b0d40a}
...
\label{sec:org54f5119}

\begin{itemize}
\item Example
\label{sec:org25d6087}
\begin{itemize}
\item Original
\label{sec:orgdbd2028}
...
\label{sec:orgd01f535}
\begin{itemize}
\item :BMCOL:
\label{sec:orgf03bd46}
The SWAP gate swaps the information between qubits.

\item Dependence graph
\label{sec:org3af42c7}
...
\label{sec:orgbd4d74c}


\begin{itemize}
\item Example definition
\label{sec:org7ddac12}
\begin{itemize}
\item Example definition
\label{sec:org83ced04}
\begin{center}
...
\label{sec:org4e8bbf4}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgb5cd2c3}
...
\item :B\(_{\text{onlyenv}}\):
\label{sec:orgdaa9e10}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org50e4a3c}
\begin{center}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:orge941b3e}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org8ef66c4}
...
\label{sec:org948d69b}
\begin{itemize}
\item Example definition
\label{sec:org77bd82d}
\begin{itemize}
\item Example definition
\label{sec:org81a5649}
\begin{center}
...
\label{sec:orgfe1c5f7}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org34c1f66}
\begin{center}
...
\item :BMCOL:
\label{sec:org2eeabc4}



\item Gray Code Quantum Circuit
\label{sec:org4aaa408}
\uline{Gray Code Quantum Circuit}
...
\label{sec:orgda23084}
\begin{itemize}
\item Example definition
\label{sec:org29505a0}
\begin{itemize}
\item Example definition
\label{sec:org71cc168}
\begin{center}
...
\item :BMCOL:
\label{sec:orgdcfc860}



\item Gray Code Quantum Circuit
\label{sec:orgf3e8632}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgaf93416}
\begin{center}
\resizebox{\textwidth}{!}{
...
\subsection*{Mapping example. Routing and re-scheduling}
\label{sec:org6a9a33a}
\begin{itemize}
\item Example definition
\label{sec:orgd7c7b83}
\begin{itemize}
\item Example definition
\label{sec:orgd578d5b}
\begin{center}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org9ed8ea9}
\begin{itemize}
\item :B\(_{\text{structureenv}}\):
\label{sec:org8dfc226}
\begin{center}
...
\item :BMCOL:
\label{sec:org043b6d7}



\item Gray Code Quantum Circuit
\label{sec:org615e4fb}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org3e03791}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org97f8432}
\begin{center}
$\Box$ \text{Cycle}
\end{center}

\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgd16058e}
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
...
\label{sec:orgd84e46a}

\begin{itemize}
\item Circuits
\label{sec:org7222782}
\begin{itemize}
\item :BMCOL:
\label{sec:org75ba555}
      \begin{center}
...
\label{sec:org9fd90a2}

\begin{itemize}
\item :BMCOL:
\label{sec:org1ae36bc}
\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org92db151}
Error sources:
...
\item :BMCOL:
\label{sec:org68cbb24}
\begin{itemize}
\item Windows error image
\label{sec:org9c76d1b}
\vspace{.5cm}
...
\item :B\(_{\text{ignoreheading}}\):
\label{sec:org31d610f}

\item Best Mapping
\label{sec:org2465fd4}
Ideal mapping should not inject extra errors.

\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org945ed62}
...
\label{sec:org9ea3525}

\begin{itemize}
\item Index
\label{sec:org65ae7d3}
\begin{itemize}
\item What is the people doing?
\label{sec:org462b931}
\small
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org89eacf4}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org39a2c08}
...
\label{sec:org6088cae}

\begin{itemize}
\item SC-17 topology
\label{sec:orgac01694}
\begin{itemize}
\item Images
\label{sec:orgdc4ccac}
\begin{center}
\includegraphics[width=.6\textwidth]{figs/sc-17.eps}
\end{center}

\item Images
\label{sec:org2e7c8ff}
...
\item Constraints
\label{sec:org6b245ac}
\begin{itemize}
\item :BMCOL:
\label{sec:org5c1e2ee}
Constraints:
