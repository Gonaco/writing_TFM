...
First, in the \hyperref[sec:org076d2bd]{Impact of mapping in the overall circuit error} section we show how the mapping procedure affects the general increment of error.
After that, we analyze the metrics and their correlation with the error increase in the \hyperref[sec:org08d6562]{Analysis of the mapping metrics} section.
Finally, in the \hyperref[sec:org499fbf3]{Advice} section, after a regression study, we offer a way to predict the amount of errors a circuit could get depending on the its characteristics.

\section{Impact of mapping in the overall circuit error}
\label{sec:org076d2bd}
...
In Fig. \ref{fig:org765d4db} we highlight the difference in fidelity of some benchmarks before and after being mapped.
...
\caption{\label{fig:org765d4db}
Difference of fidelities before and after mapping with the \texttt{minextendrc} router for five different benchmarks.}
\end{figure}
In Fig. \ref{fig:org9f58dd6} we can see [explain the points that I'm plotting and the selection of them as well as what we are trying to show]
...
In Fig. \ref{fig:org61bed1f} we plot the results of the framework in terms of probability of success and fidelity. 
