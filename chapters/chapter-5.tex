...
First, in the \hyperref[sec:orgb5a15c2]{Impact of mapping in the overall circuit error} section we show how the mapping procedure affects the general increment of error.
After that, we analyze the metrics and their correlation with the error increase in the \hyperref[sec:org8f8d45e]{Analysis of the mapping metrics} section.

\section{Impact of the mapping on the algorithm reliability}
\label{sec:orgb5a15c2}
...
\begin{itemize}
\item The fidelity difference tends to grow. The longer the circuit the bigger difference in fidelity [Make sense because the long circuits the more it will require more routing and, therefore, it will add more depth]
\end{itemize}
