...
First, in the \hyperref[sec:orge9b3249]{Impact of mapping in the overall circuit error} section we show how the mapping procedure affects the general increment of error.
After that, we analyze the metrics and their correlation with the error increase in the \hyperref[sec:org76a7108]{Analysis of the mapping metrics} section.
Finally, in the \hyperref[sec:org26ee608]{Advice} section, after a regression study, we offer a way to predict the amount of errors a circuit could get depending on the its characteristics.
...
In Fig. \ref{fig:orgc12435e} we highlight the difference in fidelity of some benchmarks before and after being mapped.
...
\caption{\label{fig:orgc12435e}
Difference of fidelities before and after mapping with the \texttt{minextendrc} router for five different benchmarks.}
\end{figure}
In Fig. \ref{fig:org075bd3a} we can see [explain the points that I'm plotting and the selection of them as well as what we are trying to show]
...
In Fig. \ref{fig:orgc8c0130} we plot the results of the framework in terms of probability of success and fidelity. 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figures/f_ps_correlation_with_meas_error.eps}
\caption{\label{fig:orgc8c0130}
