...
First, in the \hyperref[sec:org3179a22]{Impact of mapping in the overall circuit error} section we show how the mapping procedure affects the general increment of error.
After that, we analyze the metrics and their correlation with the error increase in the \hyperref[sec:org0a11d3d]{Analysis of the mapping metrics} section.
Finally, in the \hyperref[sec:org3806f05]{Regression analysis} section, after a regression study, we offer a way to predict the amount of errors a circuit could get depending on the its characteristics.

\section{Impact of mapping in the overall circuit error}
\label{sec:org3179a22}
...
In Fig. \ref{fig:orga02c60d} we highlight the difference in fidelity of some benchmarks before and after being mapped.
...
\caption{\label{fig:orga02c60d}
Difference of fidelities before and after mapping with the \texttt{minextendrc} router for five different benchmarks.}
\end{figure}
In Fig. \ref{fig:orgdb31f79} we can see [explain the points that I'm plotting and the selection of them as well as what we are trying to show]
...
In Fig. \ref{fig:org8eb18fa} we plot the results of the framework in terms of probability of success and fidelity. 
