...
First, in the \hyperref[sec:orgc55af0f]{Impact of mapping in the overall circuit error} section we show how the mapping procedure affects the general increment of error.
After that, we analyze the metrics and their correlation with the error increase in the \hyperref[sec:org7bed3f1]{Analysis of the mapping metrics} section.

\section{Impact of mapping in the overall circuit error}
\label{sec:orgc55af0f}
...
Finally, it can be seen how the number of gates is the metric most related with the fidelity, as predicted by the Pearson coefficient; it is the one with the samples more ordered.
