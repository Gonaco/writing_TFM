...
\caption{\label{fig:org769bd3c}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:org094e583}
...
In eq. \ref{eq:orge51ec89} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:org967c78c}.
Eq. \ref{eq:org0be64ba} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:org957cddb}
Algorithm Quantum Volume as in eq. \ref{eq:org1e8092b} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:org61386e6}
Algorithm Quantum Volume as in eq. \ref{eq:org1e8092b} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:orgef257a3}
Algorithm Quantum Volume as in eq. \ref{eq:org81e5646} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:org5490c14}
