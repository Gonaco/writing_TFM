...
\caption{\label{fig:org4147d85}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:org34f65fb}
...
In eq. \ref{eq:org0aeb0f8} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:org3fc766b}.
Eq. \ref{eq:org1b81c60} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:org30ee9ab}
Algorithm Quantum Volume as in eq. \ref{eq:orgcf27fe1} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:org85f467d}
Algorithm Quantum Volume as in eq. \ref{eq:orgcf27fe1} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:org496ea32}
Algorithm Quantum Volume as in eq. \ref{eq:org41a022d} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:org9d5a49b}
