...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:orgc8777fd}.
Eq. \ref{eq:org815a036} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:org1609883}
Algorithm Quantum Volume as in eq. \ref{eq:org8554839} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:org7cc0df1}
Algorithm Quantum Volume as in eq. \ref{eq:org8554839} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:orgec77cf0}
Algorithm Quantum Volume as in eq. \ref{eq:orgc5ed5da} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:orge563cfb}
