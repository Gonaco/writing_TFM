...
\caption{\label{fig:org17be0d4}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:org74972f1}
...
In eq. \ref{eq:org4840d82} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:org90293ba}.
Eq. \ref{eq:org9a8d836} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:orgd332804}
Algorithm Quantum Volume as in eq. \ref{eq:org3f57a70} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:org95da6af}
Algorithm Quantum Volume as in eq. \ref{eq:org3f57a70} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:orgb1587fd}
Algorithm Quantum Volume as in eq. \ref{eq:orga2f48fc} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:org0e7335b}
