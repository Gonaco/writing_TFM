...
\caption{\label{fig:org8f1c243}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:orgfe0028a}
...
\label{sec:orge90cd47}
\subsection*{Initial placement}
\label{sec:org7ae717c}
\subsection*{Router}
\label{sec:org8514c0c}
\subsection*{Resource constrains scheduler}
\label{sec:org7647ebc}
...
In eq. \ref{eq:orgbde8386} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:orgc277b21}.
Eq. \ref{eq:org97e984d} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:orgcdd713b}
Algorithm Quantum Volume as in eq. \ref{eq:org77a8084} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:org8c7e0a5}
Algorithm Quantum Volume as in eq. \ref{eq:org77a8084} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:org61331ef}
Algorithm Quantum Volume as in eq. \ref{eq:orgbf0e894} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:org3fceb27}
