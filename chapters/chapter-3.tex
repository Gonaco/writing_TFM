...
\caption{\label{fig:org47db45b}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:org5ee94ea}
...
In eq. \ref{eq:orgb8fb74e} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:org264f18e}.
Eq. \ref{eq:org78d9c01} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:org533cd2f}
Algorithm Quantum Volume as in eq. \ref{eq:org656ff08} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:orgc0ee022}
Algorithm Quantum Volume as in eq. \ref{eq:org656ff08} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:org5a737c1}
Algorithm Quantum Volume as in eq. \ref{eq:org2448301} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:org402c74b}
