...
Quantum Volume is metric developed by IBM to depict whether a device is able to run a quantum circuit or not.
...
Consequently we will use it as a metric to measure the runnability of the quantum algorithms and the quantum devices.
...
I our case, we want to relate the Quantum Volume of a device with the circuit.
...
As with \(V_Q\), we initially defined the algorithm's Quantum Volume from the general equation \(V_Q\) (see eq. \ref{eq:orgc799c10}), although we will adapt it later.
