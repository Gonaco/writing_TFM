...
\caption{\label{fig:org406ecf6}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:orgcea12a5}
...
In eq. \ref{eq:org366cc2f} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:org852555a}.
Eq. \ref{eq:org2066836} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:org047b804}
Algorithm Quantum Volume as in eq. \ref{eq:org0bd7f8d} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:org980590d}
Algorithm Quantum Volume as in eq. \ref{eq:org0bd7f8d} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:org69fdb88}
Algorithm Quantum Volume as in eq. \ref{eq:org4eea9f1} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:orgb8d7e1b}
