...
\caption{\label{fig:orgd5ee79e}
Mapping flow}
\end{figure}

\subsection*{Circuit decomposition}
\label{sec:orgd5f50b0}
...
In eq. \ref{eq:org941339f} the fidelity formula is shown to understand its behaviour.
...
But, for a situation with one of the states as pure state (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}\)) the calculation of fidelity reduces its complexity as it can be understood from eq. \ref{eq:orgea2dc59}.
Eq. \ref{eq:org2e6c705} the next step of simplicity, the one in which both states are pure (\({\displaystyle \rho =|\psi _{\rho }\rangle \!\langle \psi _{\rho }|}, {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|} {\displaystyle \sigma =|\psi _{\sigma }\rangle \!\langle \psi _{\sigma }|}\)).
...
\caption{\label{fig:org137d74f}
Algorithm Quantum Volume as in eq. \ref{eq:org2066885} behaviour for values less than \(10^{4}\) qubits and depth less than \(10^{4}\)}
\end{figure}
...
\caption{\label{fig:orgdcf9217}
Algorithm Quantum Volume as in eq. \ref{eq:org2066885} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}
...
\caption{\label{fig:orga86327f}
Algorithm Quantum Volume as in eq. \ref{eq:orgb0fa826} behaviour for values less than 1000 qubits and a depth less than \(10^{5}\)}
\end{figure}

\item Runnability
\label{sec:org0602a02}
