...
In the \hyperref[sec:orgd0910e7]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:orgb49ef92]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orgb37a549]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:orgd0910e7}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orgb37a549]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
\begin{itemize}
\item cQASM
\label{sec:orgef0ae51}


\item OpenQL
\label{sec:org5b0566e}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:orga86e02f}
OpenQL diagram}
\end{figure}
...
\label{sec:orge7733b9}

\begin{itemize}
\item Error model parameters
\label{sec:orge137f31}

\begin{table}[htbp]
\caption{\label{tab:org5511dea}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:orgfc4380c}) occurs due to the \hyperref[sec:org34f7df4]{photon decay} effect.
