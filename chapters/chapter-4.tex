...
In the \hyperref[sec:org77cb9c3]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org0f2eac3]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orga7fd2a0]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org77cb9c3}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orga7fd2a0]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
\begin{itemize}
\item cQASM
\label{sec:orgf5dcb20}


\item OpenQL
\label{sec:org9c83602}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org62ad490}
OpenQL diagram}
\end{figure}
...
\label{sec:org76c8b54}

\begin{itemize}
\item Error model parameters
\label{sec:org39eadc5}

\begin{table}[htbp]
\caption{\label{tab:orgab192d3}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org4708411}) occurs due to the \hyperref[sec:org1fb512d]{photon decay} effect.
