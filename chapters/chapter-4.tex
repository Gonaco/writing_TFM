...
In the \hyperref[sec:org9e6db32]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:orga08dd54]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orgf242aeb]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org9e6db32}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orgf242aeb]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
\begin{itemize}
\item cQASM
\label{sec:org590132c}


\item OpenQL
\label{sec:orgb8790e3}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org9dfe524}
OpenQL diagram}
\end{figure}
...
\label{sec:orgf6221ed}

\begin{table}[htbp]
\caption{\label{tab:org9ad42a3}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:orgecc84b4}) occurs due to the \hyperref[sec:org774de69]{photon decay} effect.
