...
In the \hyperref[sec:org5fe3cce]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org6d06b2b]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:org62d262d]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org5fe3cce}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:org62d262d]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:orgdd2bc7a}.
...
In Tab. \ref{tab:orgdd2bc7a} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:org88f1959}


\item OpenQL
\label{sec:org8bf851c}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org6d69258}
OpenQL diagram}
\end{figure}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org21b83d1}) occurs due to the \hyperref[sec:org8be1928]{photon decay} effect.
