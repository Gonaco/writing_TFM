...
In the \hyperref[sec:org67452aa]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org3eeaded]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:org1747c80]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org67452aa}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:org1747c80]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:orgc032d3e}.
...
In Tab. \ref{tab:orgc032d3e} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:org3ef7d87}


\item OpenQL
\label{sec:orgc986be8}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org83c7b9e}
OpenQL diagram}
\end{figure}
...
\label{sec:org3960ac5}

\begin{itemize}
\item Error model parameters
\label{sec:orgce455af}

\begin{table}[htbp]
\caption{\label{tab:org9662928}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:orgd05f1fb}) occurs due to the \hyperref[sec:org7b20248]{photon decay} effect.
