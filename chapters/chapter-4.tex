...
In the \hyperref[sec:org6c5a769]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org2b0ea0c]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orgf33fe45]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org6c5a769}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orgf33fe45]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:orgabc0457}.
...
In Tab. \ref{tab:orgabc0457} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:org66c38d9}


\item OpenQL
\label{sec:org37c4a2b}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:orga545591}
OpenQL diagram}
\end{figure}
...
Also, Tab. \ref{tab:orgf09e99f} pinpoints exactly the value of the parameters used in the error model.


\begin{itemize}
\item Qubit Idling
\label{sec:org25ed427}
...
This dephasing is then implemented via the same Pauli transfer matrix as \(R_{\Lambda_{T_{\phi}}}\) in eq. \ref{eq:orgd5c4a04}.

\item Flux Noise
\label{sec:org55ae4a8}
...
Quantumsim considers this shrinking with a factor of \(1 - p_{axis}\) along the y-axis and \(1 - p_{plane}\) along the x- and z-axes as it is described in eq. \ref{eq:org8114a2d}.
