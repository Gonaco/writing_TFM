...
In the \hyperref[sec:org3087391]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org47d286c]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:org22c310e]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org3087391}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:org22c310e]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:org1aebc0f}.
...
In the next figures, from Fig. \ref{fig:orge1582db} to Fig. \ref{code:q_gate_bench}, we show the different steps and data of the analysis in order to do the selection.
In Tab. \ref{tab:org1aebc0f} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:org308b19c}


\item OpenQL
\label{sec:orgdb75d99}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org3583a9a}
OpenQL diagram}
\end{figure}
...
Also, Tab. \ref{tab:orgf4741de} pinpoints exactly the value of the parameters used in the error model.


\begin{itemize}
\item Qubit Idling
\label{sec:org1771eda}
...
This dephasing is then implemented via the same Pauli transfer matrix as \(R_{\Lambda_{T_{\phi}}}\) in eq. \ref{eq:orgb3b1ec1}.

\item Flux Noise
\label{sec:org1019769}
...
Quantumsim considers this shrinking with a factor of \(1 - p_{axis}\) along the y-axis and \(1 - p_{plane}\) along the x- and z-axes as it is described in eq. \ref{eq:org5d36b35}.
