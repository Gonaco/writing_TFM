...
In the \hyperref[sec:org6c6cb6a]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org0f23d02]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:org0aa410b]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org6c6cb6a}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:org0aa410b]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:orge42f7db}.
...
In Tab. \ref{tab:orge42f7db} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:orga2744f6}


\item OpenQL
\label{sec:org55c296b}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:orgd9090b6}
OpenQL diagram}
\end{figure}
...
\label{sec:org463c294}

\begin{itemize}
\item Error model parameters
\label{sec:orge1d8332}

\begin{table}[htbp]
\caption{\label{tab:org6f44276}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org0b796ec}) occurs due to the \hyperref[sec:org51d861b]{photon decay} effect.
