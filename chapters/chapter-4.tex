...
In the \hyperref[sec:org8b72e9a]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org96679c0]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:org3eeae9b]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org8b72e9a}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:org3eeae9b]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:orgd5a39e4}.
...
In Tab. \ref{tab:orgd5a39e4} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:org1af4873}


\item OpenQL
\label{sec:org2e50080}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:orgb033bf1}
OpenQL diagram}
\end{figure}
...
\label{sec:org5b2f98d}

\begin{table}[htbp]
\caption{\label{tab:orgef86ac7}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org3554a47}) occurs due to the \hyperref[sec:org1c832e5]{photon decay} effect.
