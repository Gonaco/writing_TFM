...
In the \hyperref[sec:orgd142f4c]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org7f64f5b]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orgb8d6724]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:orgd142f4c}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orgb8d6724]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:org7b515eb}.
...
In Tab. \ref{tab:org7b515eb} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:orgbb2494d}


\item OpenQL
\label{sec:org2c54e64}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org6793843}
OpenQL diagram}
\end{figure}
...
\label{sec:org019240d}

Quantumsim is a quantum superconducting systems simulator designed to study the SC-7 and SC-17 chips.
It performs calculations on density matrices
It has the possibility of boosting calculations with a Graphics Processing Unit (GPU).



\begin{itemize}
\item Error model parameters
\label{sec:org841a45e}

\begin{table}[htbp]
\caption{\label{tab:org7fce2c0}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org2b525ac}) occurs due to the \hyperref[sec:orgbd6463b]{photon decay} effect.
