...
In the \hyperref[sec:org302271f]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org5540150]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orga0be1da]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org302271f}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orga0be1da]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
For example, two similar algorithms which number of qubits or number of gates are very distinct as the first three selected benchmarks in Tab. \ref{tab:org0d96571}.
...
In the next figures, from Fig. \ref{fig:orgef0f905} to Fig. \ref{code:q_gate_bench}, we show the different steps and data of the analysis in order to do the selection.
In Tab. \ref{tab:org0d96571} we show the final benchmark selection.
...
\begin{itemize}
\item cQASM
\label{sec:orgb2c6939}


\item OpenQL
\label{sec:orgaaf2b21}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org1342c84}
OpenQL diagram}
\end{figure}
...
Also, Tab. \ref{tab:org29d46a8} pinpoints exactly the value of the parameters used in the error model.


\begin{itemize}
\item Qubit Idling
\label{sec:orgae9ee83}
...
This dephasing is then implemented via the same Pauli transfer matrix as \(R_{\Lambda_{T_{\phi}}}\) in eq. \ref{eq:org34577db}.

\item Flux Noise
\label{sec:orgc8b283e}
...
Quantumsim considers this shrinking with a factor of \(1 - p_{axis}\) along the y-axis and \(1 - p_{plane}\) along the x- and z-axes as it is described in eq. \ref{eq:org1d2756e}.
