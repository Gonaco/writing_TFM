...
In the \hyperref[sec:org19e0675]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org5450d60]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orgea0b289]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org19e0675}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orgea0b289]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
\begin{itemize}
\item cQASM
\label{sec:orgd6a8a3d}


\item OpenQL
\label{sec:orgfe595fb}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:org37b9e07}
OpenQL diagram}
\end{figure}
...
\label{sec:org478ac54}

\begin{itemize}
\item Error model parameters
\label{sec:orgba8f244}

\begin{table}[htbp]
\caption{\label{tab:org90b6135}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org0377030}) occurs due to the \hyperref[sec:org9e6944e]{photon decay} effect.
