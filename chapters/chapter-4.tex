...
In the \hyperref[sec:org12e7909]{Benchmarks} section we describe how we built the algorithm set.
In the \hyperref[sec:org11a0ab8]{Error framework} section we describe the structure of the mapping of the algorithms procedure and its posterior metric calculation.
...
OpenQL will be explained in much detail in the section \hyperref[sec:orge792ddc]{Compiler (OpenQL, cQASM)?}.

\section*{Benchmarks}
\label{sec:org12e7909}
...
As mentioned in the introduction of this chapter and as it is shown in Section \hyperref[sec:orge792ddc]{Compiler (OpenQL, cQASM)?}, in order to map the algorithms we will use the OpenQL tool.
...
\begin{itemize}
\item cQASM
\label{sec:org82e7837}


\item OpenQL
\label{sec:orga4c0283}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:orge6f6952}
OpenQL diagram}
\end{figure}
...
\label{sec:org445ffc5}

\begin{table}[htbp]
\caption{\label{tab:org8b94187}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org784351d}) occurs due to the \hyperref[sec:org1bfb962]{photon decay} effect.
