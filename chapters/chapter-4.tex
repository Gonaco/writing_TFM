...
\label{sec:org2db81f9}
\section*{Benchmarks}
\label{sec:org7b55bdc}

\subsection*{Benchmarks formation and classification}
\label{sec:org64127e3}
\begin{itemize}
\item Benchmarks formation and classification
\label{sec:orge323456}

\begin{itemize}
\item Objective
\label{sec:org1d546b8}
...
\item Benchmarks formation and classification
\label{sec:org4e240d3}

\begin{itemize}
\item Objective
\label{sec:orgbf047c2}
...
\item {\bfseries\sffamily TODO} :B\(_{\text{noteNH}}\):
\label{sec:org3daeefc}
First, to build the algorithms list.
\end{itemize}

\item Benchmarks formation and classification
\label{sec:org66a0d13}

\begin{itemize}
\item :B\(_{\text{ignoreheading}}\):
\label{sec:orgd46d2e7}
...
\label{sec:org10988c5}
\begin{itemize}
\item :B\(_{\text{onlyenv}}\):
\label{sec:org76c68af}
\begin{itemize}
\item OPENQASM
\begin{itemize}
\item RevLib (55)
...
\label{sec:org6c3cf81}

\begin{itemize}
\item Statistics
\label{sec:org250916c}
...
\item Benchmarks formation and classification
\label{sec:org3a865c2}

\begin{itemize}
\item Objective
\label{sec:orgd26159f}

Gather a useful list of benchmarks from RevLib \cite{Wille_2008}, ScaffCC \cite{JavadiAbhari_2015}, Zulehner's paper \cite{zulehner17:effic_method_mappin_quant_circuit} and QLib \cite{Lin_2014} in order to use them later for the "probability of success" analysis.


\item :B\(_{\text{ignoreheading}}\):
\label{sec:org9f4cbd4}
Steps:
...
\label{sec:org11f9c1b}
\subsection*{Compiler (OpenQL, cQASM)?}
\label{sec:orgf3ee5e7}
...
\begin{itemize}
\item cQASM
\label{sec:org16d6527}


\item OpenQL
\label{sec:org47aff76}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/openql.png}
\caption{\label{fig:orgade5ce9}
OpenQL diagram}
\end{figure}
...
\label{sec:org8de6d28}

\begin{itemize}
\item Error model parameters
\label{sec:org077638c}

\begin{table}[htbp]
\caption{\label{tab:orged14fae}
...
The initial dephasing step in the measurement model (Fig. \ref{fig:org7a762e5}) occurs due to the \hyperref[sec:org82c1023]{photon decay} effect.
